...
import src2.features
import src2.ModelCnn
...
def get_train_validation(x1, x2, x3, x4, y, fen):
    ss = 2
    ...
    train_x4 = [x4[i] for i in range(len(x4)) if i % fen != ss]
    valid_x4 = [x4[i] for i in range(len(x4)) if i % fen == ss]
    train_y = [y[i] for i in range(len(y)) if i % fen != ss]
    valid_y = [y[i] for i in range(len(y)) if i % fen == ss]
    return train_x1, train_x2, train_x3, train_x4, train_y, valid_x1, valid_x2, valid_x3, valid_x4, valid_y
...
def batch_iter(x, x2, x3, x4, y, batch_size=64):
    """生成批次数据"""
    data_len = len(x)
    ...
    x4_shuffle = np.array(x4)[indices]
    y_shuffle = np.array(y)[indices]

    for i in range(num_batch):
        start_id = i * batch_size
        end_id = min((i + 1) * batch_size, data_len)
        yield x_shuffle[start_id:end_id], x2_shuffle[start_id:end_id], x3_shuffle[start_id:end_id], x4_shuffle[start_id:end_id], y_shuffle[start_id:end_id]

def feed_data(model, x1_batch, x2_batch, x3_batch, x4_batch, y_batch, keep_prob, w2v_matrix, learn_rate):
    feed_dict = {
        model.input_voc_x: x1_batch,
        model.input_w2v_x: x2_batch,
        model.input_f2_x: x3_batch,
        model.input_cix_x: x4_batch,
...
def evaluate(sess, model, x1_, x2_, x3_, x4_, y_, w2v_matrix, learn_rate):
    """评估在某一数据上的准确率和损失"""
    data_len = len(x1_)
    batch_eval = batch_iter(x1_, x2_, x3_, x4_, y_, 128)
    total_loss = 0.0
    total_acc = 0.0
    for x1_batch, x2_batch, x3_batch, x4_batch, y_batch in batch_eval:
        batch_len = len(x1_batch)
        feed_dict = feed_data(model, x1_batch, x2_batch, x3_batch, x4_batch, y_batch, 1.0, w2v_matrix, learn_rate)
